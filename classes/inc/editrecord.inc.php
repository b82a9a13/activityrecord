...
if(!preg_match($number, $courseep) || empty($courseep)){
    array_push($errorarray, ['courseep', get_string('course_eptd_short', $p).':'.preg_match($numberReplace,'', $courseep)]);
}
...
if(!preg_match($textarea, $recap)){
    array_push($errorarray, ['recap', get_string('recap_act', $p).':'.preg_replace($textareaReplace,'', $recap)]);
}
$recapimpact = $_POST['recapimpact'];
if(!preg_match($textarea, $recapimpact)){
    array_push($errorarray, ['recapimpact', get_string('what_impact', $p).':'.preg_replace($textareaReplace,'', $recapimpact)]);
}
$details = $_POST['details'];
if(!preg_match($textarea, $details) || empty($details)){
    array_push($errorarray, ['details', get_string('details_ot', $p).':'.preg_replace($textareaReplace,'', $details)]);
}
$detailsmod = $_POST['detailsmod'];
if(!preg_match($textarea, $detailsmod) || empty($detailsmod)){
    array_push($errorarray, ['detailsmod', get_string('modules_aksb', $p).':'.preg_replace($textareaReplace,'', $detailsmod)]);
}
$impact = $_POST['impact'];
if(!preg_match($textarea, $impact) || empty($impact)){
    array_push($errorarray, ['impact', get_string('what_impact', $p).':'.preg_replace($textareaReplace,'', $impact)]);
}
...
if(!preg_match($textarea, $agreedact) || empty($agreedact)){
    array_push($errorarray, ['agreedact', get_string('agreed_act', $p).':'.preg_replace($textareaReplace,'', $agreedact)]);
}
$fileArr = [];
$file = null;
if(isset($_FILES['file'])){
    ...
    if(!empty($file['name'])){
        ...
        if(in_array($fileext, ['pdf'])){
            ...
            array_push($errorarray, ['file', get_string('file_pdfo', $p)]);
        }
    }
}
$apprencom = $_POST['apprencom'];
if(!preg_match($textarea, $apprencom)){
    array_push($errorarray, ['apprencom', get_string('apprentice_com', $p).':'.preg_replace($textareaReplace,'', $apprencom)]);
}
$coachsigndate = $_POST['coachsigndate'];
if($coachsigndate != null && !empty($coachsigndate)){
    if(!preg_match($date, $coachsigndate)){
        array_push($errorarray, ['coachsigndate', get_string('coach_sd', $p)]);
    } else {
...
if(!preg_match($textarea, $hands) || empty($hands)){
    array_push($errorarray, ['hands', get_string('health_as', $p).':'.preg_replace($textareaReplace, '', $hands)]);
}
$eandd = $_POST['eandd'];
if(!preg_match($textarea, $eandd) || empty($eandd)){
    array_push($errorarray, ['eandd', get_string('equality_ad', $p).':'.preg_replace($textareaReplace, '', $eandd)]);
}
$iaag = $_POST['iaag'];
if(!preg_match($textarea, $iaag) || empty($iaag)){
    array_push($errorarray, ['iaag', get_string('information_aag', $p).':'.preg_replace($textareaReplace, '', $iaag)]);
}

if($errorarray != []){
    $returnText->error = $errorarray;
} else {
    $result = $lib->update_activity_record([
        $apprentice,
        $reviewdate,
        $standard,
        $eors,
    ...
    if($result){
        ...
        \local_activityrecord\event\updated_activity_record::create(array('context' => \context_course::instance($_SESSION['ar_records_cid']), 'courseid' => $_SESSION['ar_records_cid'], 'relateduserid' => $_SESSION['ar_records_uid']))->trigger();
    } else {
