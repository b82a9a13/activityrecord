<?php
/**
 * Strings for component 'local_activityrecord', language 'en'
 * @package   local_activityrecord
 * @author      Robert Tyrone Cullen
 */
//A
    $string['activity_r'] = 'Activity Records';
    $string['apprentice'] = 'Apprentice';
    $string['apprentice_com_title'] = 'Apprentice comments regarding their learning journey';
    $string['apprentice_com'] = 'Apprentice comments';
    $string['aln_title'] = 'ALN (Additional Learner Needs) support delivered today';
    $string['aln'] = 'ALN';
    $string['agreed_act_title'] = 'Agreed actions & future skills development activity';
    $string['agreed_act'] = 'Agreed actions';
    $string['add_s'] = 'Add Signature';
//B
    $string['btm'] = 'Back to menu';
//C
    $string['coach'] = 'Coach';
    $string['coach_otf'] = 'Coach/Tutor Feedback';
    $string['course_c'] = 'Course Comments';
    $string['course_ptd'] = 'Course % Progress to date';
    $string['course_eptd'] = 'Course % Expected Progress according to Training Plan';
    $string['course_eptd_short'] = 'Course % Expected Progress';
    $string['comments'] = 'Comments';
    $string['choose_apdf'] = 'Choose a PDF';
    $string['coach_s'] = 'Coach Signature';
    $string['coach_sd'] = 'Coach Signature Date';
//D
    $string['date_at_title'] = 'Date & time of next planned review';
    $string['delete'] = 'Delete';
    $string['details_ot_title'] = 'Details of Teaching & Learning activity undertaken today (include reference to modules and knowledge, skills & behaviours)';
    $string['details_ot'] = 'Details of Teaching & Learning';
//E
    $string['edit'] = 'Edit';
    $string['employer_os'] = 'Employer or Store';
    $string['employer_cop'] = 'Employer comment on progress';
    $string['expected_otjh_aptp'] = 'Expected OTJH as per Training Plan';
    $string['expected_otjh'] = 'Expected OTJH';
    $string['equality_ad'] = 'Equality and Diversity';
    $string['edit_ar'] = 'Edit Activity Record';
    $string['english_lt'] = 'English, Learning today';
    $string['english_t'] = 'English, Target';
//F
    $string['functional_sp'] = 'Functional Skills Progress';
    $string['face_tf'] = 'Face to Face';
    $string['file_ms'] = 'File:max size 2.5mb';
    $string['file_i'] = 'File:invalid';
    $string['file_pdfo'] = 'File:pdf only';
    $string['full_name'] = 'Full Name';
//G
//H
    $string['health_as'] = 'Health and Safety';
//I
    $string['information_aag'] = 'Information Advice and Guidance';
    $string['invalid_i'] = 'Invalid id';
    $string['invalid_t'] = 'Invalid type';
    $string['invalid_cip'] = 'Invalid course id provided';
//J
//L
    $string['learning_t'] = 'Learning today';
    $string['learner_s'] = 'Learner Signature';
    $string['learner_sd'] = 'Learner Signature Date';
//M
    $string['manager_om'] = 'Manager/Mentor';
    $string['modules_aksb'] = 'Modules and K,S,B';
    $string['math_lt'] = 'Math, Learning today';
    $string['math_t'] = 'Math, Target';
//N
    $string['no'] = 'No';
    $string['no_da'] = 'No data available';
    $string['new_record'] = 'New Record';
    $string['next_pr'] = 'Next planned review';
    $string['no_la'] = 'No learners available';
    $string['no_cip'] = 'No course id provided';
//O
    $string['otj_a'] = 'Off The Job Activity';
    $string['otjh_c'] = 'OTJH Completed';
    $string['otjh_com'] = 'OTJH Comments';
//P
    $string['pluginname'] = 'Activity Record';
    $string['pdf'] = 'PDF';
//Q
//R
    $string['records'] = 'Records';
    $string['review_date'] = 'Review Date';
    $string['recap_act_title'] = 'Recap on actions from last month';
    $string['recap_act'] = 'Recap on actions';
    $string['remote_ftf'] = 'Remote / Face to Face';
    $string['remote'] = 'Remote';
    $string['required_vns'] = 'Required variables are not set';
//S
    $string['standard'] = 'Standard';
    $string['summary_op'] = 'Summary of progress';
    $string['safeguarding'] = 'Safeguarding';
    $string['submit'] = 'Submit';
//T
    $string['target_fnv'] = 'Target for next visit';
    $string['tracking'] = 'Tracking';
//U
//V
//W
    $string['what_impact_title'] = 'What impact has this had in you current job role/situation';
    $string['what_impact'] = 'What impact';
    $string['what_impactw_title'] = 'What impact will this have in your job role/situation';
//X
//Y
    $string['yes'] = 'Yes';
//Z